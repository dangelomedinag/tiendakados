...
const errLogs = (variant) => {
  ...
  if (variant.includes('.jpg') || variant.includes('.png')) {
    checkFiles.isImage = true;
  } else {
    checkFiles.isImage = false;
    checkFiles.errors.push({
      variant: variant,
      type: variant.slice(-4),
      log: 'extensión de archivo invalida',
    });
  }
  ...
  if (
    ...
    checkFiles.typeProduct = true;
    // product.type = variant.split('_')[0];
  } else {
    checkFiles.typeProduct = false;
    checkFiles.errors.push({
      variant: variant,
      type: variant.split('_')[0],
      log: 'Tipo de producto no reconocido',
    });
  }
  ...
  if (variant.includes('_.')) {
    ...
    checkFiles.errors.push({
      variant: variant,
      type: variant.slice(-5),
      log:
        'El nombre del archivo no puede finalizar con "_" (guión bajo) justo antes de la extension',
    });
  }
  if (variant.includes('ñ')) {
    let strfail = variant.charAt(variant.indexOf('ñ'));
  ...
  if (variant.includes(' ')) {
    ...
    checkFiles.errors.push({
      variant: variant,
      type:
        variant.slice(0, variant.indexOf(' ')) +
        ' [--AQUI HAY UN ESPACIO--] ' +
        variant.slice(variant.indexOf(' ') + 1),
      log:
        'Nombre de archivo no puede contener espacios ej: "Polera_negra_fanstasma Mario" puede agregar un "_" o "-" para solucionarlo',
    });
  }
...
const detectColor = (variant, type) =>{
  // assing color for product
