<script>
  ...
  let appstate = {
    ...
    API_url: 'http://localhost:3000/api/all',
...
<!-- anuncio de copiado -->
<Snackbar bind:visible={appstate.show_element.snackbar} bottom="true" bg="rgb(25, 118, 210)">
  Copiado al Portapapeles<span slot="action">
    <Button color="#ff0" on:click={() => (appstate.show_element.snackbar = false)}>Cerrar</Button>
  </span>
</Snackbar>

<!-- selector de collecciones y tools -->
<Sidepanel disableScroll="true" left bind:visible={appstate.show_element.menu_left}>
    ...
    {#if appstate.data}
      <div class="button-side">
        <Button
          ...
          shaped="true"
          fullWidth="true">
          HOMBRE todos ({appstate.data_master.filter(el => el.type.includes('hombre')).length})
        </Button>
        ...
        <Button
          ...
          shaped="true"
          fullWidth="true">
          MUJER todos ({appstate.data_master.filter(el => el.type.includes('mujer')).length})
        </Button>
        <hr style="margin-bottom: 8px;" />
        {#each appstate.collections as categ (categ)}
          <Button
...
<div class="topbar">
  ...
  <Button on:click={() => (appstate.show_element.searchModal = !appstate.show_element.searchModal)} color="primary">
    {#if appstate.data}
...
<main>
  <div class="tablewrapper">
    {#if appstate.data}
      <Table data={appstate.data} sizes={appstate.sizes} htmlview={appstate.show_element.htmlview} urlimages={appstate.shopify_img_url} viewimg={appstate.show_element.viewimg}/>
    {:else}
      <Spinner />
    {/if}
  </div>
...
<div class="bottombar">
  <div class="container-row">
    <div class="d-flex d-col">
      <ColorSelector colors={productsconfig.colores.array} favorite={productsconfig.polera_H.colorfavs} />
      <ColorSelector colors={productsconfig.colores.array_poleron} favorite={productsconfig.polerones.colorfavs} />
      ...
      <Textfield
        ...
        <div>
          {#if appstate.data}
              ...
              required
              type="text"
              value={appstate.data.map(item => item.type).reduce((arr,item) => arr.includes(item) ? arr : [...arr, item], []).join(', ')}
              disabled=true
